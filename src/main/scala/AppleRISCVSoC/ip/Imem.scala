...
case class Imem() extends Component {
  ...
  val imem_cpu_sib = slave(Sib(SIBCfg.imemSibCfg))
  val imem_dbg_sib = slave(Sib(SIBCfg.imemSibCfg))
  ...
  // == CPU side Port == //
  val cpu_word_addr = imem_cpu_sib.addr(SOCCfg.INSTR_RAM_ADDR_WIDTH - 1 downto 2)
  ...
  // == DBG side Port == //
  val dbg_word_addr = imem_dbg_sib.addr(SOCCfg.INSTR_RAM_ADDR_WIDTH - 1 downto 2)
