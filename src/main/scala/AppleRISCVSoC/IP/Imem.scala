...
case class Imem() extends Component {
  ...
  val imem_cpu_sib = slave(Sib(AppleRISCVSoCCfg.imemSibCfg))
  val imem_dbg_sib = slave(Sib(AppleRISCVSoCCfg.imemSibCfg))

  // We fetch  a word (4 bytes) at a time so reduce the size by 4
  val SIZE = 1 << (AppleRISCVSoCCfg.INSTR_RAM_ADDR_WIDTH - 2)
  val ram = new Mem(Bits(AppleRISCVSoCCfg.INSTR_RAM_DATA_WIDTH bits), SIZE)

  // == CPU side Port == //
  val cpu_word_addr = imem_cpu_sib.addr(AppleRISCVSoCCfg.INSTR_RAM_ADDR_WIDTH - 1 downto 2)
  ...
  // == DBG side Port == //
  val dbg_word_addr = imem_dbg_sib.addr(AppleRISCVSoCCfg.INSTR_RAM_ADDR_WIDTH - 1 downto 2)
