...
def print_register(dut, size=32):
    ...
    for i in range(size):
        try:
            val = dut.DUT_AppleRISCVSoC.soc_cpu_core.core_idStage.regFile_1.regFileRam[i].value.integer
            print(f"Register {i}, {hex(val)}")
        except ValueError:
            print(f"Register {i}, X")


def check_register(dut, expected):
    """ Check the register file with the expected data """
    for key, value in expected.items():
        val = dut.DUT_AppleRISCVSoC.soc_cpu_core.core_idStage.regFile_1.regFileRam[key].value.integer
