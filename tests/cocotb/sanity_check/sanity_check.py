...
def clear_imem(dut, imem_size_to_clear, soc):
    ...
    for i in range(imem_size_to_clear):
        ...
        if soc == 'de2':
            dut.DUT_AppleRISCVSoC.soc_imem_inst.ram.altsyncram_component.mem_data[i] = 0
            #dut.DUT_AppleRISCVSoC.soc_imem_inst.ram.altsyncram_component.mem_data_b[i] = 0
    print(f"[TB-INFO]: Cleaning the Instruction Memory done")

def load_imem(dut, imem, soc):
    ...
    for i in range(size):
        ...
        if soc == 'de2':
            dut.DUT_AppleRISCVSoC.soc_imem_inst.ram.altsyncram_component.mem_data[i]   = imem[i]
            #dut.DUT_AppleRISCVSoC.soc_imem_inst.ram.altsyncram_component.mem_data_b[i] = imem[i]
...
async def run_test(dut, imm_data, expected_register, runtime=400, imem_size_to_clear=100):
    clear_imem(dut, imem_size_to_clear, soc)
    load_imem(dut, imm_data, soc)
...
import b2b_lw
@cocotb.test()
async def test_b2b_lw(dut):
    """ Test the back to back load word logic """
    await run_test(dut, b2b_lw.imem_data, b2b_lw.expected_register, 2000)